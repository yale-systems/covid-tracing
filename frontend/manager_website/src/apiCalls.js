...
export default {
    ...
    async managerCheckLogin(credentials) {
        const userRegex = /^username[1-3]?[0-9]$/;
        const passwordRegex = /^password$/;
        if (credentials.username.match(userRegex) &&
            credentials.password.match(passwordRegex)) {
            ...
            const Id = credentials.username.match(numberRegex);
            // 2. CALL getPatient
            const url = baseurl + 'manager/getbymanagerid/'
            return await this.getManager(url, Id[0]) 
        } else {
            return null
        }
    },
    ...
    async getPatients(url) {
      ...
      await axios.get(url)
        ...
        console.log(res)
        return {patients: res, links: links}
    },
    ...
    async getVolunteers(link) {
        ...
        await axios.get(link)
            ...
            .catch(error => console.error(error))
        return {volunteers: res, links: links}
    },

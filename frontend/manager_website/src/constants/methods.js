    ...
    methods: {
        gettersHelper(item, field) {
            const gettersMap = {
                ...
                'phone': (item) => this.processRetValue(item.phone_number, 'phone_number'),
                'email': (item) => this.processRetValue(item.email, 'email'),
                'age' : (item) => item.age,
                ...
                'dob': (item) => item.date_of_birth.format('MM/DD/YYYY'),
                ...
                'contact_date': (item) => item.contact_date.format('MM/DD/YYYY'),
                'update_date': (item) => item.update_date.format('MM/DD/YYYY'),
                'relationship': (item) => this.getEnum(item, 'Relationship'),
            
            
                //patient-specific case info
                'diagnosis_date': (item) => item.diagnosis_date.format('MM/DD/YYYY'),
                'onset_date': (item) => item.onset_date.format('MM/DD/YYYY'),
                'last_worked_date': (item) => item.last_worked_date.format('MM/DD/YYYY'),
                ...
                'symptomatic': (item) => {
                    if (typeof item.symptomatic == 'boolean') {
                        let ret = this.processRetValue(item.symptomatic, 'symptomatic')
                        if (typeof ret == 'boolean') { return ret == true ? "Yes" : "No" }
                        return ret
                    }
                ...
                'requested_interview': (item) => {return item.requested_interview ? "Yes" : "No"},
        ...
        processRetValue(ret, field) {
            return (ret == null || ret == undefined || ret == '') ? `missing ${field.replace(/_/g, ' ')}` : ret
        },
        getEnum(item, field) {
            let underscore = this.camelToUnderscore(field)
            let ret = this.enums[field].find((el) => el.key == item[underscore]).value
            return this.processRetValue(ret, underscore)
        },
        getEnumArray(item, field) {
            let underscore = this.camelToUnderscore(field)
            if(item[underscore] === undefined || item[underscore] == null || item[underscore].length == 0) {
                return 'None'
            } else {
                let str = ''
